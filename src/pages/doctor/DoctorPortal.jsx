...
const todayStats = [
  { label: 'Patients Seen', value: '8', color: 'var(--color-primary-500)', bg: 'var(--color-primary-50)' },
  { label: 'Prescriptions', value: '11', color: 'var(--color-secondary-500)', bg: '#eff6ff' },
  { label: 'Alerts Fired', value: '2', color: 'var(--color-severe)', bg: 'var(--color-severe-bg)' },
  { label: 'Safe Checks', value: '9', color: 'var(--color-safe)', bg: 'var(--color-safe-bg)' },
]

const recentPatients = [
  { name: 'Ravi Kumar', abha: '14-1234-5678-9012', time: '10:30 AM', status: 'Completed', level: 'safe' },
  { name: 'Priya Singh', abha: '14-9876-5432-1098', time: '11:00 AM', status: 'Completed', level: 'moderate' },
  { name: 'Amit Patel', abha: '14-5555-6666-7777', time: '11:45 AM', status: 'In Progress', level: null },
]
    ...
    <PageShell sidebar={<Sidebar items={sidebarItems} role="doctor" />}>
      <div className="dashboard">
        <div className="dashboard__topbar">
          <div>
            <h1 className="dashboard__greeting">Dr. Sharma 🩺</h1>
            <p className="dashboard__date">Cardiologist · Monday, April 14, 2025</p>
          </div>
          <div className="dashboard__topbar-actions">
            <button className="dashboard__notif-btn"><Bell size={20} /><span className="dashboard__notif-dot" /></button>
            <div className="dashboard__avatar" style={{ background: 'var(--color-primary-800)' }}>DS</div>
          </div>
        </div>
            ...
            {recentPatients.map((p, i) => (
              <div key={i} className="appointment-item">
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                  <div className="dashboard__avatar" style={{ width: 36, height: 36, fontSize: 'var(--text-xs)' }}>{p.name.split(' ').map(n => n[0]).join('')}</div>
                  <div>
                    <strong style={{ fontSize: 'var(--text-sm)' }}>{p.name}</strong>
                    <span style={{ display: 'block', fontSize: 'var(--text-xs)', color: 'var(--color-gray-400)', fontFamily: 'var(--font-mono)' }}>{p.abha}</span>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                  <span style={{ fontSize: 'var(--text-sm)', color: 'var(--color-gray-400)' }}>{p.time}</span>
                  {p.level && <span className={`badge badge-${p.level}`}>{p.level}</span>}
                  <span className={`badge ${p.status === 'Completed' ? 'badge-safe' : 'badge-teal'}`}>{p.status}</span>
                </div>
              </div>
            ))}
          </div>
